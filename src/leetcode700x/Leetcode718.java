...
Time Complexity: O(M*N*\min(M, N))O(M∗N∗min(M,N)), where M, NM,N are the lengths of A, B. The worst case is when all the elements are equal.

Space Complexity: O(N)O(N), the space used by Bstarts. (Of course, we could amend our algorithm to make this O(\min(M, N))O(min(M,N)).)
...
​​ +p
​n−1
​​ x. This shows we can get the hash of all A[i:i+\text{guess}]A[i:i+guess] in linear time. We will also use the fact that p^{-1} = p^{\mathcal{M}-2} \mod \mathcal{M}p
​−1
​​ =p
​M−2
​​ modM.
