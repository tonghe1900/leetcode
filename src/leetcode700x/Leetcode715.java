...
Time Complexity: Let KK be the number of elements in ranges. addRange and removeRange operations have O(K)O(K) complexity. queryRange has O(\log K)O(logK) complexity. Because addRange, removeRange adds at most 1 interval at a time, you can bound these further. For example, if there are AA addRange, RR removeRange, and QQ queryRange number of operations respectively, we can express our complexity as O((A+R)^2 Q \log(A+R))O((A+R)
​2
​​ Qlog(A+R)).
