...
public class QuickSums {
  public  int minSums(String numbers, int sum){
	  Map<Integer, Set<Integer>> map = possibleSums(numbers, 0, numbers.length()-1, new HashMap<String, Map<Integer, Set<Integer>>>());
  ...
  private Map<Integer, Set<Integer>>  possibleSums(String numbers, int begin, int end, Map<String, Map<Integer, Set<Integer>>> map){
	...
	if(map.containsKey(key)) {
	  ...
	  set.add(0);
	   ...
	   result.put(Integer.parseInt(numbers.substring(begin, end+1)), set);
	 
	
	  for(int i= begin+1; i<= end; i+=1){
		 
		  Map<Integer, Set<Integer>>  firstResult = possibleSums(numbers, begin, i-1, map);
		  Map<Integer, Set<Integer>>  secondResult = possibleSums(numbers, i, end, map);
...
private void process(Map<Integer, Set<Integer>> result,
		Map<Integer, Set<Integer>> firstResult,
		Map<Integer, Set<Integer>> secondResult, Integer firstKey,
		Integer secondKey, int sumVal, boolean addPlus) {
...
public static void main(String [] args){
	System.out.println( new QuickSums().minSums("11", 2));   // 1
	System.out.println( new QuickSums().minSums("0123456789", 45));  //8
	System.out.println( new QuickSums().minSums("99999", 45));   //4
	System.out.println( new QuickSums().minSums("1110", 3));   //3
	System.out.println( new QuickSums().minSums("99999", 100));  //-1
	System.out.println( new QuickSums().minSums("382834", 100));  //2
	//System.out.println( new QuickSums().minSums("9230560001", 71)); //4
	System.out.println( new QuickSums().minSums("0000", 71));
	System.out.println( new QuickSums().minSums("0000", 0));
	System.out.println( new QuickSums().minSums("0100", 1));
	System.out.println( new QuickSums().minSums("000000000007", 7));
