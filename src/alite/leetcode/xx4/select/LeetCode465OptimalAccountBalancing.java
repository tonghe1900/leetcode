...
A group of friends went on holiday and sometimes lent each other money. F
or example, Alice paid for Bill's lunch for $10. Then later Chris gave Alice $5 for a taxi ride.
...
public class LeetCode465OptimalAccountBalancing {
	public int minTransfers(int[][] transactions) {
	    ...
	    return helper1(a, 0, cnt);
	}
	
	int helper1(int[] a, int start, int n) {
		if(start == n) return 0;
	    int ans = Integer.MAX_VALUE;
	    while(start < n && a[start] == 0) start++;
	    if(start == n) return 0;
	    for (int i = start + 1; i < n; ++i) {
	        if (a[i] < 0 && a[start] > 0 || a[i] > 0 && a[start] < 0) {
	            a[i] += a[start];
	            ans = Math.min(ans, helper1(a, start + 1, n) +1);
	            a[i] -= a[start];
	        }
	    }
