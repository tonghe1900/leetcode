...
//					 System.out.println();
	...
	public static int ways4(int[] coins, int total) {
		if (coins == null || coins.length == 0) {
			return total == 0 ? 1 : 0;
		}
		if (total == 0)
...
//					 System.out.println();
	...
	// -1
	public static int ways3(int[] coins, int total) {
		if (coins == null || coins.length == 0) {
			return total == 0 ? 1 : 0;
		}
		if (total == 0)
		...
		for (int i = 1; i <= total; i += 1) {
			...
			for (int j = 1; j <= coins.length; j += 1) {
				...
				for (int k = 0; k* coins[j-1] <= i; k += 1) {
					//if (i >= coins[k - 1]) {
						ways[i][j] += ways[i - k* coins[j - 1]][j-1];
					//}
...
//		}
	...
	public static void main(String[] args) {
		...
		System.out.println(ways3(new int[] { 1, 5, 10, 25 }, 250));
