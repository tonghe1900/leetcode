...
public class LeetCode340 {
	 public int lengthOfLongestSubstringKDistinct(String s, int k) {
	        ...
	        for (int j = 0; j < s.length(); j++) {
	            if (count[s.charAt(j)]++ == 0) num++;
	            if (num > k) {   //while (--count[s.charAt(i++)] > 0);//while (--count[s.charAt(i++)] > 0);
	                while (--count[s.charAt(i++)] > 0);//??   while (--count[s.charAt(i++)] > 0);//??  while (--count[s.charAt(i++)] > 0);//??   、、while (--count[s.charAt(i++)] > 0);
	                num--;
	            }
	            res = Math.max(res, j - i + 1);
	        }
