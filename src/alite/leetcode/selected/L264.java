...
这道题就是LeetCode 313. Super Ugly Number的简化版本，毕竟那道题是super ugly。具体解法可以看我曾经写过的Super Ugly Number的解答，当然这里的primes只有三个，完全不必要使用priorityqueue（用了反而超时了）,O(n)的space用来dp储存前n个ugly numbers，时间也是O(n)，因为每次dp找下一个ugly number只需要O(1)的操作。为了方便读者，我这里复制黏贴一下我之前Super Ugly Number的解答，稍作优化既可达到上文说的复杂度。
