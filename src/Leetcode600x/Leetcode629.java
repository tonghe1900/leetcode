...
Time complexity : O(n^2*k)O(n
​2
​​ ∗k). dpdp of size nnxkk is filled once. Filling each dpdp entry takes O(n)O(n) time.
...
Thus, we can directly obtain dp[i][j]dp[i][j] value as dp[i][j] = dp[i-1][j] - dp[i-1][j-i] + dp[i-1][j]dp[i][j]=dp[i−1][j]−dp[i−1][j−i]+dp[i−1][j], if j−i≥0. Otherwise, we can use: dp[i][j] = dp[i-1][j] + dp[i-1][j]dp[i][j]=dp[i−1][j]+dp[i−1][j].
